...
//     };
//     theThing = {
//       longStr: new Array(2000000).join('*'),
//       someMethod: function(){
...
// };
//
// app.get('/leak',function(req,res,next){
//   replaceThing();
...
var startServer = () => {
  ...
  //  console.log('(%s) started on port (%d) in (%s) mode; press Ctrl+C to terminate', appInfo.name, app.get('port'), app.get('env'));
    console.log('(%s) started on port (%d) in (%s) mode; press Ctrl+C to terminate', appInfo.name, app.get('port'));
  });
};
